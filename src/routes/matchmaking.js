...
router.post('/join', authenticate, matchmakingRateLimit, async (req, res, next) => {
  try {
    ...
    if (err.message.includes('déjà en file') || err.message.includes('match en cours')) {
...
router.get('/active-match', authenticate, async (req, res, next) => {
  try {
    const { PrismaClient } = require('@prisma/client');
    const prisma = new PrismaClient();
    const participant = await prisma.matchParticipant.findFirst({
      ...
      include: {
        match: {
          select: { id: true, mode: true, queueType: true, status: true, startedAt: true },
        },
      },
    });
    res.json({ match: participant ? participant.match : null });
  } catch (err) { next(err); }
});
